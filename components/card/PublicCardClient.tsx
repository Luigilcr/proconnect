 ...
 *
 * CLIENTE INTERACTIVO PARA PERFIL PÚBLICO DE TARJETA NFC (/c/[slug])
 */
...
interface PublicCardClientProps {
  slug: string;
  initialCard?: FullCard | null;
}

export function PublicCardClient({ slug, initialCard }: PublicCardClientProps) {
  ...
  const [card, setCard] = useState<FullCard | null>(initialCard || getCachedCard(cleanSlug));
  ...
  useEffect(() => {
    ...
    const fetchCard = async () => {
      ...
      if (!foundCard) {
        foundCard = getCardBySlug(cleanSlug);
      }

      // 3. Fallback a API del servidor
      if (!foundCard) {
        try {
          const res = await fetch(`/api/cards?slug=${encodeURIComponent(cleanSlug)}`);
      ...
      // 4. Procesar y guardar en caché instantáneo
      if (foundCard) {
        ...
        try {
          if (typeof window !== 'undefined') {
            sessionStorage.setItem(`proconnect_card_${cleanSlug}`, JSON.stringify(foundCard));
            localStorage.setItem(`proconnect_card_${cleanSlug}`, JSON.stringify(foundCard));
          }
        } catch {}
      }
  ...
  // Verificar si la empresa matriz está suspendida (Kill Switch)
  ...
  // Verificar si la tarjeta ha expirado por fecha de corte de suscripción
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white p-6 text-center">
        ...
        <p className="text-xs text-slate-400 max-w-sm mb-6">
          No existe una tarjeta digital activa con el identificador &quot;{cleanSlug || 'desconocido'}&quot;. Verifica el enlace NFC o el código QR escaneado.
        </p>
        <Link
          href="/"
  ...
  // Si la empresa matriz está suspendida
  ...
  // Si la tarjeta ha expirado por tiempo de suscripción (30 días de corte)
  if (isCardExpired) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white p-6 text-center">
